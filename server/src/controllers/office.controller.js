...
const base = createCrudController(officeRepository, 'Office', 'offices');
...
export const listPublic = async (req, res) => {
  const offices = await Office.find({ isActive: true, isDeleted: false }).sort('order').lean();
  return new ApiResponse(200, offices).send(res);
};
