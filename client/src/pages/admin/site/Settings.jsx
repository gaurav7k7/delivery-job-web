...
function SettingsForm({ initialValues }) {
      ...
      Object.entries(data)
        .map(([key, value]) => {
          if (value && typeof value === "object" && !Array.isArray(value)) {
            return [key, omitNullValues(value)];
          }
          return [key, value];
        })
  ...
  async function onSubmit(values) {
    try {
      const payload = omitNullValues(values);
      const data = await updateSettings.mutateAsync(payload);
      if (data) {
        reset({ ...emptyValues, ...data });
      }
    } catch (error) {
      // Handled by useUpdateSiteSettings onError
    }
  }
  ...
  return (
    <>
      ...
      >
        ...
        <Card className="flex flex-col gap-4 p-6">
          <h3 className="font-heading text-h4 text-neutral-900">
            Theme colors
          </h3>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-5">
            ...
            <FormInput
              label="Background"
              placeholder="#FFFFFF"
              {...register("themeColors.background")}
            />
            <FormInput
              label="Text"
              placeholder="#111827"
              {...register("themeColors.text")}
            />
          </div>
        </Card>
          ...
          <Controller
            ...
            render={({ field }) => (
              <FormSwitch
                ...
                checked={field.value}
                onChange={field.onChange}
                hint="Hides the public site behind a maintenance message"
              />
            )}
          />
          <FormTextarea
